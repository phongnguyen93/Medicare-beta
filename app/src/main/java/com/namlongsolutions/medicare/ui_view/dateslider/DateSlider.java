...
package com.namlongsolutions.medicare.ui_view.dateslider;
...
public class DateSlider extends Dialog {
    ...
    public DateSlider(Context context, int layoutID, OnDateSetListener l, Calendar initialTime) {
    	this(context,layoutID,l,initialTime, null, null, 1,null);
    }
    
    public DateSlider(Context context, int layoutID, OnDateSetListener l, Calendar initialTime, int minInterval) {
    	this(context,layoutID,l,initialTime, null, null, minInterval,null);
    }
    
    public DateSlider(Context context, int layoutID, OnDateSetListener l,
            Calendar initialTime, Calendar minTime, Calendar maxTime, String limit) {
    	this(context,layoutID,l,initialTime, minTime, maxTime, 1,limit);
    }
    
    public DateSlider(Context context, int layoutID, OnDateSetListener l,
            Calendar initialTime, Calendar minTime, Calendar maxTime, int minInterval,String limit) {
        super(context);
        ...
        if (minInterval>1) {
        	int minutes = minTime.get(Calendar.MINUTE);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mWorktime = (TextView) findViewById(R.id.mWorktimeText);
        setLimit(limit);
        mContainer.setOnTimeChangeListener(onTimeChangeListener);
        mContainer.setMinuteInterval(minuteInterval);
        mContainer.setTime(minTime);
        if (minTime!=null) mContainer.setMinTime(minTime);
        if (maxTime!=null) mContainer.setMaxTime(maxTime);

        TextView okButton = (TextView) findViewById(R.id.dateSliderOkButton);
        okButton.setOnClickListener(okButtonClickListener);

        TextView cancelButton = (TextView) findViewById(R.id.dateSliderCancelButton);
